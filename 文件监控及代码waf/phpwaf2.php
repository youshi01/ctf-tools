...
//phpwaf检测数据包并记录攻击数据包
//引入方式如下两种，任选其一：
//1.在所需页面中加入【require_once("phpwaf.php");】 
//2.修改php.ini设置auto_prepend_file = "【phpwaf.php完整路径】";

/*
检测请求方式，除了get之外拦截下来并写日志。
*/
...
function write_attack_log($alert){
    ...
    $ffff = fopen('/tmp/request_log.txt', 'a'); //日志路径 
    fwrite($ffff, $data);  
    fclose($ffff);
    if($alert == 'GETFLAG'){
        echo "flag{asoidfjasdfm3iu94jflknakdfjnasdf}"; //如果请求带有flag关键字，显示假的flag。（2333333）
    }
    exit(0);
}
