...
static AnalyzeVBlock(ea,blnum) {
  ...
  set_name(ea,form("rscVinfoBlSize_%ld",blnum));
  create_word(ea);
  op_num(ea,0);

  ea = ea + 2;
  vsize = get_wide_word(ea);
  set_name(ea,form("rscVinfoValSize_%ld",blnum));
  create_word(ea);
  op_num(ea,0);

  ea = ea + 2;
  keyea = ea;
  set_name(key,form("rscVinfoKey_%ld",blnum));
  key = "";
  ...
  set_name(vea, form("rscVinfoValue_%ld",blnum));
...
//  msg("At %lX key is: %s\n",keyea,key);
  ...
  while ( (blend-blstart) < block ) {   // nested block exist
    ...
    set_cmt(blend, form("Nested block...%ld",blnum), 0);
