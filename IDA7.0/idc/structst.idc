  ...
  for ( idx=get_first_struc_idx(); idx != -1; idx=get_next_struc_idx(idx) ) {
    ...
    for ( m = 0;
      ...
      if ( mname == "" ) {
        ...
        else if ( type == FF_STRUCT   ) type = form("Structure '%s'",get_struc_name(get_member_strid(id,m)));
        ...
        type = get_member_flag(id,m);
             if ( isOff0(type)  ) msg(" Offset");
        else if ( isChar0(type) ) msg(" Character");
        else if ( isSeg0(type)  ) msg(" Segment");
        else if ( isDec0(type)  ) msg(" Decimal");
        else if ( isHex0(type)  ) msg(" Hex");
        else if ( isOct0(type)  ) msg(" Octal");
        else if ( isBin0(type)  ) msg(" Binary");
        msg("\n");
      }
    }
  }
