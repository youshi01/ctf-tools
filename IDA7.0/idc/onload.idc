...
static DriverLoaded(void)
{
  ...
  while ( 1 )
  {
    ...
    set_name(x,form("NextDevice_%ld",i));
    create_word(x);
    op_num(x,0);
    ...
    set_name(x+4,form("DevAttr_%ld",i));
    create_word(x+4);
    op_num(x+4,0);
    ...
    set_name(x+6,form("Strategy_%ld",i));
    create_word(x+6);
    op_plain_offset(x+6,0,get_inf_attr(INF_MIN_EA));

    set_name(x+8,form("Interrupt_%ld",i));
    create_word(x+8);
    OpOffset(x+8,get_inf_attr(INF_MIN_EA));

    set_name(x+0xA,form("DeviceName_%ld",i));
    ...
    set_name( strt, form("Strategy_Routine_%ld",i));
    set_name( intr, form("Interrupt_Routine_%ld",i));
     ...
     && get_wide_word(strt+3)==get_wide_word(strt+8)-2)
    {
     ...
     set_name(x+DevReq, form("DevRequest_%ld",i));
    }
